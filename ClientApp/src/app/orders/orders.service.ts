...
export class OrdersService {
  ...
  constructor(private service: DataService, private basketService: BasketWrapperService, @Inject('BASE_URL') baseUrl: string) {
    this.ordersUrl = baseUrl;
    }

    getOrders(): Observable<IOrder[]> {
        let url = this.ordersUrl + 'orders';
    ...
    cancelOrder(orderNumber: number): Observable<any> {
        let url = this.ordersUrl + 'orders/cancel';
    ...
    getOrder(id: number): Observable<IOrderDetail> {
        let url = this.ordersUrl + 'orders/' + id;
